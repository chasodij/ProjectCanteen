...
namespace ProjectCanteen.DAL.FluentValidation
{
    public class IngredientDTOValidator : AbstractValidator<IngredientDTO>
    {
        ...
        {
            RuleFor(ingredient => ingredient.Id)
                .MustAsync(async (id, cancellation) => await _unitOfWork.IngredientRepository
                    .GetFirstOrDefaultAsync(x => x.Id == id) != null)
                .WithMessage((ingredient, restrictionId) => $"There are no ingredient with such id: {restrictionId}");
